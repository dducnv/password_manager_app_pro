...
class CircularFloatingButtonMenu extends StatefulWidget {
  ...
  @override
  final GlobalKey<CircularFloatingButtonMenuState>? key;

  /// list of CircularMenuItem contains at least two items.
  final List<CircularFloatingButtonMenuItem> items;
  ...
  const CircularFloatingButtonMenu({
    required this.items,
  ...
  })  : assert(items.length > 0, 'items can not be empty list'),
        assert(items.length > 1, 'if you have one item no need to use a Menu'),
        super(key: key);

  @override
  CircularFloatingButtonMenuState createState() =>
      CircularFloatingButtonMenuState();
}

class CircularFloatingButtonMenuState extends State<CircularFloatingButtonMenu>
  ...
  Widget _buildMenuButton(BuildContext context) {
    return Positioned.fill(
      child: Align(
        alignment: widget.alignment,
        child: CircularFloatingButtonMenuItem(
          icon: null,
